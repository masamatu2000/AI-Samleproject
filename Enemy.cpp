...
void Enemy::Update()
{
	...
	if (prog_timer < 0.0f)
	{
		...
		case UP:
			...
			if (stage->GetMapData((newPos.x) / ENEMY_DRAW_SIZE, (newPos.y - ENEMY_DRAW_SIZE) / ENEMY_DRAW_SIZE) == 1) {
				if (stage->GetMapData((newPos.x - ENEMY_DRAW_SIZE) / ENEMY_DRAW_SIZE, newPos.y / ENEMY_DRAW_SIZE) != 1) {
					dir_ = LEFT;
				}
				else {
					dir_ = RIGHT;
